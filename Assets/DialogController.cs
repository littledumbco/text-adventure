...
public class DialogController : MonoBehaviour {
	...
	void Update () {
		...
		state_cell ();
			...
			state_door ();
		} else if (myState == States.sheets_key) {
			state_sheets_key ();
		} else if (myState == States.cell_key) {
			state_cell_key ();
		} else if (myState == States.sheets_lock) {
			state_sheets_lock ();
		} else if (myState == States.mirror_lock) {
			state_mirror_lock ();
		} else if (myState == States.door_lock) {
			state_door_lock ();
		} else if (myState == States.door_key) {
			state_door_key ();
		} else if (myState == States.cell_escape) {
			state_cell_escape ();
		}
	}
	...
	void state_cell_key () {
		...
		if (Input.GetKeyDown (KeyCode.M)) {
			myState = States.mirror_lock;
		} else if (Input.GetKeyDown (KeyCode.S)){
			myState = States.sheets_lock;
		} else if (Input.GetKeyDown (KeyCode.D)){
			myState = States.door_lock;
		}
	}
